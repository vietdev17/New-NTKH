  ...
  if (!p) {
    return (
      <div className="container-custom py-20 text-center">
        ...
        <Button asChild><Link href="/products">Quay Lại Sản Phẩm</Link></Button>
      </div>
    );
  }
  ...
  return (
    <div className="container-custom py-6 lg:py-10">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-500 mb-6 overflow-x-auto no-scrollbar">
        <Link href="/" className="hover:text-primary-600 shrink-0">Trang chủ</Link>
        <span>/</span>
        <Link href="/products" className="hover:text-primary-600 shrink-0">Sản phẩm</Link>
        {category && (
          <>
            <span>/</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-4 flex-wrap text-sm">
            {rating.average > 0 && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-gray-500">({rating.count} đánh giá)</span>
              </div>
            )}
            {p.totalSold > 0 && (
              <span className="text-gray-500 flex items-center gap-1">
                <Package className="h-3.5 w-3.5" />
                Đã bán {p.totalSold.toLocaleString()}
              </span>
            )}
          ...
          {colors.length > 0 && (
            <div>
              <p className="text-sm font-semibold text-gray-900 mb-2.5">
                Màu sắc {selectedColor && <span className="text-primary-600 font-normal">— {selectedColor.name}</span>}
              </p>
          ...
          {dimensions.length > 0 && (
            <div>
              <p className="text-sm font-semibold text-gray-900 mb-2.5">
                Kích thước {selectedDim && <span className="text-primary-600 font-normal">— {selectedDim.label}</span>}
              </p>
              <div className="flex flex-wrap gap-2">
                {dimensions.map((dim) => {
          ...
          {/* === Quantity === */}
          <div>
            <p className="text-sm font-semibold text-gray-900 mb-2.5">Số lượng</p>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              onClick={() => {
                toggle(p._id);
                toast.success(wished ? 'Đã xóa khỏi yêu thích' : 'Đã thêm vào yêu thích!');
              }}
              className={cn(
          ...
          <div className="grid grid-cols-3 gap-3 pt-3">
            {[
              { icon: Truck, title: 'Miễn phí vận chuyển', desc: 'Trong bán kính 150km' },
              { icon: Shield, title: 'Bảo hành chính hãng', desc: 'Theo nhà sản xuất' },
      ...
      <div className="mt-12 lg:mt-16">
        <Tabs defaultValue="description">
          <TabsList className="mb-6 bg-gray-50 p-1 rounded-xl">
            <TabsTrigger value="description" className="rounded-lg">Mô Tả Chi Tiết</TabsTrigger>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-1">
                ...
                {dimensions.map((dim) => (
                  <div key={dim.id} className="flex justify-between py-3 border-b border-gray-100">
                    <span className="text-sm text-gray-500">Kích thước ({dim.label})</span>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b">
                      ...
                      <th className="text-left px-4 py-3 font-semibold text-gray-700">Màu sắc</th>
                      <th className="text-left px-4 py-3 font-semibold text-gray-700">Kích thước</th>
      ...
      {relatedProducts.length > 0 && (
        <div className="mt-12 lg:mt-16">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl lg:text-2xl font-bold text-gray-900">Sản Phẩm Liên Quan</h2>
            {category && (
